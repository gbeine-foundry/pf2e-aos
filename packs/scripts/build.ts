...
const packsPF2eDataPath = path.resolve(__dirname, "../../../pf2e/packs/data");
const packDirPaths = fs.readdirSync(packsDataPath).map((dirName) => path.resolve(__dirname, packsDataPath, dirName))
    .concat(fs.readdirSync(packsPF2eDataPath).map((dirName) => path.resolve(__dirname, packsPF2eDataPath, dirName)));
